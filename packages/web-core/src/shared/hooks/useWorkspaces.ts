...
async function fetchWorkspaceSummariesByArchived(
  ...
  try {
    const response = await makeLocalApiRequest('/api/workspaces/summary', {
      method: 'POST',
...
export function useWorkspaces(): UseWorkspacesResult {
  ...
  const activeEndpoint = '/api/workspaces/stream/ws?archived=false';
  const archivedEndpoint = '/api/workspaces/stream/ws?archived=true';
