...
const loadEmptyPrPreference = (): boolean => {
  try {
    const stored = localStorage.getItem(EMPTY_PR_PREFERENCE_KEY);
    return stored === 'true';
  } catch {
    return false;
  }
...
const saveEmptyPrPreference = (value: boolean): void => {
  try {
    localStorage.setItem(EMPTY_PR_PREFERENCE_KEY, String(value));
  } catch {
...
const CreatePRDialogImpl = create<CreatePRDialogProps>(
  ({ attempt, repoId, targetBranch, issueIdentifier }) => {
    ...
    const [createEmptyPr, setCreateEmptyPr] = useState(loadEmptyPrPreference());
    const resetFormState = useCallback(() => {
      ...
      setCreateEmptyPr(loadEmptyPrPreference());
    }, [config?.pr_auto_description_enabled]);

    const handleEmptyPrChange = useCallback(
      (checked: boolean | 'indeterminate') => {
        const isChecked = checked === true;
        setCreateEmptyPr(isChecked);
        saveEmptyPrPreference(isChecked);
      },
      []
    );

    // Handle mutual exclusivity and field clearing
    useEffect(() => {
      if (createEmptyPr) {
        // When empty PR is enabled, set placeholder title, clear body, disable auto-generate
        // GitHub/Azure APIs require non-empty titles, so we use a single space
        setPrTitle(' ');
        setPrBody('');
    ...
    useEffect(() => {
      if (autoGenerateDescription) {
        // When auto-generate is enabled, disable empty PR
        setCreateEmptyPr(false);
      }
    ...
    useEffect(() => {
      ...
      const initializePRFields = async () => {
        // Use minimal placeholder if empty PR is enabled
        // GitHub/Azure APIs require non-empty titles
        if (createEmptyPr) {
          setPrTitle(' ');
          setPrBody('');
          return;
    ...
    }, [attempt.id, modal.visible, isLoaded, issueIdentifier, createEmptyPr]);
            ...
            {!isLoaded ? (
              ...
              <div className="space-y-4 py-4">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="pr-empty"
                    checked={createEmptyPr}
                    onCheckedChange={handleEmptyPrChange}
                    className="h-5 w-5"
                  />
                  <Label htmlFor="pr-empty" className="cursor-pointer text-sm">
                    {t('createPrDialog.emptyPrLabel')}
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    disabled={createEmptyPr}
                    className="h-5 w-5"
                  />
                  <Label
                    htmlFor="pr-auto-generate"
                    className={`cursor-pointer text-sm ${createEmptyPr ? 'opacity-50' : ''}`}
                  >
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    disabled={autoGenerateDescription || createEmptyPr}
                    className={
                      autoGenerateDescription || createEmptyPr
