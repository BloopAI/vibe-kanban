...
const fs = require("fs");
const { ensureBinary, BINARY_TAG, CACHE_DIR, LOCAL_DEV_MODE, LOCAL_DIST_DIR, GITHUB_REPO, getLatestVersion } = require("./download");
...
async function main() {
  ...
  const hasValidRepo = !GITHUB_REPO.startsWith("__");
  if (!isMcpMode && !LOCAL_DEV_MODE && hasValidRepo) {
    getLatestVersion()
      .then((latest) => {
        if (latest && latest !== CLI_VERSION) {
          setTimeout(() => {
            console.log(`\nUpdate available: ${CLI_VERSION} -> ${latest}`);
            console.log(`Run: npx vibe-kanban-pm@latest`);
          }, 2000);
        }
      })
      .catch(() => {});
  ...
  if (isMcpMode) {
    ...
    console.log(`Starting vibe-kanban-pm v${CLI_VERSION}${modeLabel}...`);
