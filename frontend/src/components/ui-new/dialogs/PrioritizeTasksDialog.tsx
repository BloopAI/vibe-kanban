...
import { ChartBarIcon, SparkleIcon, ArrowsDownUpIcon } from '@phosphor-icons/react';
...
const priorityMethods: { value: PriorityMethod; label: string; description: string; icon: typeof ChartBarIcon }[] = [
  {
    value: 'ai',
    label: 'AI-Assisted',
...
const PrioritizeTasksDialogImpl = NiceModal.create<PrioritizeTasksDialogProps>((props) => {
      ...
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader>
          ...
          <DialogDescription className="text-left pt-2">
            Automatically assign priorities to {taskCount > 0 ? `${taskCount} tasks` : 'your tasks'}.
          </DialogDescription>
        </DialogHeader>

        <div className="py-4">
          <label className="text-sm font-medium mb-3 block">Prioritization Method</label>
          <div className="space-y-2">
            {priorityMethods.map(({ value, label, description, icon: Icon }) => (
              <button
                key={value}
                type="button"
...
export const PrioritizeTasksDialog = defineModal<PrioritizeTasksDialogProps, PrioritizeTasksResult>(
