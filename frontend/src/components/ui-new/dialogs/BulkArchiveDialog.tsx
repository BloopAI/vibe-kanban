...
import { ArchiveIcon, CheckCircleIcon, WarningIcon } from '@phosphor-icons/react';
...
const archiveOptions: { value: ArchiveOption; label: string; description: string }[] = [
  {
    value: 'completed',
...
const BulkArchiveDialogImpl = NiceModal.create<BulkArchiveDialogProps>((props) => {
  const modal = useModal();
  const { completedTaskCount = 0, cancelledTaskCount = 0 } = props;
  const [selectedOption, setSelectedOption] = useState<ArchiveOption>('completed');
      ...
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader>
          ...
          <DialogDescription className="text-left pt-2">
            Move old or completed tasks to the archive to keep your board clean.
          </DialogDescription>
        </DialogHeader>

        <div className="py-4">
          <label className="text-sm font-medium mb-3 block">What to archive</label>
          <div className="space-y-2">
            {archiveOptions.map(({ value, label, description }) => {
              const count = value === 'completed'
                ? completedTaskCount
                : value === 'cancelled'
                  ? cancelledTaskCount
                  : value === 'both'
...
export const BulkArchiveDialog = defineModal<BulkArchiveDialogProps, BulkArchiveResult>(
