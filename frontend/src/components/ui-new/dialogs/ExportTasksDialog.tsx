...
import { UploadSimpleIcon, FileTextIcon, FileCsvIcon, CodeIcon } from '@phosphor-icons/react';
...
const formatOptions: { value: ExportFormat; label: string; icon: typeof FileTextIcon }[] = [
  { value: 'csv', label: 'CSV', icon: FileCsvIcon },
  { value: 'json', label: 'JSON', icon: CodeIcon },
  { value: 'markdown', label: 'Markdown', icon: FileTextIcon },
];

const ExportTasksDialogImpl = NiceModal.create<ExportTasksDialogProps>((props) => {
      ...
      <DialogContent className="sm:max-w-[400px]">
        <DialogHeader>
          ...
          <DialogDescription className="text-left pt-2">
            Export {taskCount > 0 ? `${taskCount} tasks` : 'all tasks'} to a file.
          </DialogDescription>
        </DialogHeader>
...
export const ExportTasksDialog = defineModal<ExportTasksDialogProps, ExportTasksResult>(
