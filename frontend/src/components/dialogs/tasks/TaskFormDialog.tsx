...
import { Globe2, Settings2, ArrowDown, Plus, Play, Image } from 'lucide-react';
...
export const TaskFormDialog = NiceModal.create<TaskFormDialogProps>(
  ({
    ...
    // Check if templates should be shown (only when form is mostly empty)
    ...
    // Delayed template hiding for smooth animation
    const [showTemplatesContent, setShowTemplatesContent] =
      useState(shouldShowTemplates);

    useEffect(() => {
      if (shouldShowTemplates) {
        // Show immediately when templates should appear
        setShowTemplatesContent(true);
      } else {
        // Delay hiding content to allow animation to complete
        const timer = setTimeout(() => setShowTemplatesContent(false), 300);
        ...
        >
          <DialogContent className="h-full overflow-hidden flex flex-col gap-0 p-4 pb-0">
            <div className="flex-1 overflow-y-auto space-y-4 px-0">
              ...
              <Collapse open={shouldShowTemplates}>
