...
const AutoReviewSettingsDialogImpl = NiceModal.create<AutoReviewSettingsDialogProps>(
  ({ currentSettings, onSave }) => {
    ...
    const [executorProfileId, setExecutorProfileId] = useState<ExecutorProfileId | null>(
      currentSettings.executorProfileId ?? config?.executor_profile ?? null
    );
    const [includePmReview, setIncludePmReview] = useState(currentSettings.includePmReview);
    const [includeCodeReview, setIncludeCodeReview] = useState(currentSettings.includeCodeReview);
    const [additionalPrompt, setAdditionalPrompt] = useState(currentSettings.additionalPrompt);

    useEffect(() => {
      // Reset form when dialog opens
      setEnabled(currentSettings.enabled);
      setExecutorProfileId(currentSettings.executorProfileId ?? config?.executor_profile ?? null);
        ...
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <div className="flex items-center gap-2">
              ...
              <DialogTitle>{t('autoReviewSettings.title', 'Auto-Review Settings')}</DialogTitle>
            </div>
            <DialogDescription>
              {t(
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label htmlFor="auto-review-enabled" className="text-sm font-medium">
                  {t('autoReviewSettings.enableAutoReview', 'Enable Auto-Review')}
                </Label>
              ...
              <>
                ...
                {profiles && (
                  <div className="space-y-2">
                    <Label className="text-sm font-medium">
                      {t('autoReviewSettings.selectAI', 'Select AI for Review')}
                    </Label>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setIncludePmReview(checked === true)}
                      />
                      <div className="space-y-0.5">
                        <Label htmlFor="pm-review" className="cursor-pointer text-sm">
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => setIncludeCodeReview(checked === true)}
                      />
                      <div className="space-y-0.5">
                        <Label htmlFor="code-review" className="cursor-pointer text-sm">
                ...
                <div className="space-y-2">
                  <Label htmlFor="additional-prompt" className="text-sm font-medium">
                    {t('autoReviewSettings.additionalPrompt', 'Additional Instructions (Optional)')}
                  </Label>
                  <Textarea
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave}>{t('common:buttons.save', 'Save')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    );
...
export const AutoReviewSettingsDialog = defineModal<AutoReviewSettingsDialogProps, void>(
