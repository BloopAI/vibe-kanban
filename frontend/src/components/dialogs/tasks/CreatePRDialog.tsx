...
import { useAuth, useRepoBranches, useProjectRemotes } from '@/hooks';
import {
...
const CreatePRDialogImpl = NiceModal.create<CreatePRDialogProps>(
  ({ attempt, task, repoId, targetBranch }) => {
    ...
    const { data: remotes = [], isLoading: remotesLoading } = useProjectRemotes(
      task.project_id,
      { enabled: modal.visible && !!task.project_id }
    );
    ...
    // Set default remote when remotes are loaded
    useEffect(() => {
      if (remotes.length > 0 && !selectedRemote) {
        // Prefer "origin" as default, otherwise use first remote
        const originRemote = remotes.find((r) => r.name === 'origin');
        setSelectedRemote(originRemote?.name ?? remotes[0].name);
      }
                ...
                {remotes.length > 1 && (
                  <div className="space-y-2">
                    <Label htmlFor="pr-remote">
                      {t('createPrDialog.remoteLabel', { defaultValue: 'Remote' })}
                    </Label>
                    <Select
                    ...
                    >
                      <SelectTrigger
                        id="pr-remote"
                        className={remotesLoading ? 'opacity-50 cursor-not-allowed' : ''}
                      >
                        <SelectValue
                          placeholder={
                            remotesLoading
                              ? t('createPrDialog.loadingRemotes', { defaultValue: 'Loading remotes...' })
                              : t('createPrDialog.selectRemote', { defaultValue: 'Select remote' })
                          }
                        />
                      </SelectTrigger>
                      <SelectContent>
