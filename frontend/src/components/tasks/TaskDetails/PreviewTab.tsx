...
import { TaskAttempt, ExecutionProcess } from 'shared/types';
...
function NoServerContent({
    ...
    const handleSaveDevScript = async (startAfterSave?: boolean) => {
        ...
        try {
            ...
            setSaveError(
                (err as Error)?.message || 'Failed to save dev script'
            );
        } finally {
            ...
            <div className="text-center space-y-4 max-w-md mx-auto p-6">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        {effectiveHasDevScript ? 'Please start a dev server to see the preview' : 'To use the live preview and click-to-edit, please add a dev server script to this project.'}
                    </p>
                    ...
                    {((effectiveHasDevScript && isEditingExistingScript) || !effectiveHasDevScript) && (
...
export default function PreviewTab({
    ...
    const copyPrompt = () => {
        navigator.clipboard.writeText('Please install https://github.com/BloopAI/vibe-kanban-web-companion');
    }
            ...
            <div className={`flex-1 flex flex-col min-h-0`}>
                ...
                {mode === 'ready' && loadingTimeFinished && !isReady && (
                    <Alert variant="destructive" className='space-y-2'>
                        <p className="font-bold">We're having trouble previewing your application:</p>
                        <ol className="list-decimal list-inside space-y-2">
                            <li>Did the dev server start successfully? There may be a bug you need to resolve, or perhaps dependencies need to be installed.</li>
                            <li>
                                Did your dev server print the URL and port to the terminal in the format <code>http://localhost:3000</code>?
                                (this is how we know it's running)
                            </li>
                            <li>Have you installed the Web Companion (required for click-to-edit)? If not, please <a href="https://github.com/BloopAI/vibe-kanban-web-companion" target="_blank" className="underline font-bold">follow the installation instructions here</a>.</li>
                        </ol>
                        <p className="border-2 p-2">
                            <p>Tip: you can ask your coding agent to install the Web Companion for you with the prompt: <code className="font-bold">Please install https://github.com/BloopAI/vibe-kanban-web-companion</code> <Button variant="ghost" className="p-0 h-0" onClick={copyPrompt}><Copy className="w-3 h-3" /></Button></p>
                        </p>
