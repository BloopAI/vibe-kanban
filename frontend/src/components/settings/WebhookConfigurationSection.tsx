...
interface WebhookConfigurationSectionProps {
  ...
  onUpgradeNotificationsChange: (enabled: boolean) => void;
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t(
            ...
            'Configure webhook notifications for upgrades and remote server deployments'
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center space-x-2">
          <Checkbox
            id="upgrade-notifications"
            checked={upgradeNotificationsEnabled}
            onCheckedChange={onUpgradeNotificationsChange}
          />
          <div className="space-y-0.5">
            <Label htmlFor="upgrade-notifications" className="cursor-pointer">
              {t(
                'settings.general.webhooks.upgradeNotifications.label',
                'Notify on Upgrades'
              )}
            </Label>
            <p className="text-sm text-muted-foreground">
              {t(
                'settings.general.webhooks.upgradeNotifications.helper',
                'Receive notifications when Vibe Kanban is upgraded to a new version'
              )}
            </p>
          </div>
        </div>
        ...
        <p className="text-xs text-muted-foreground">
          {t(
            ...
            'Webhooks will receive notifications for app upgrades. Configure your webhook endpoints above.'
          )}
        </p>
      </CardContent>
    </Card>
