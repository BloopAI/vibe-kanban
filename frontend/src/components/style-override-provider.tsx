...
export const StyleOverrideProvider: React.FC<StyleOverrideProviderProps> = ({
  children,
}) => {
        ...
        if (message.payload.kind === 'theme') {
          ...
          Object.entries(message.payload.variables).forEach(([name, value]) => {
            ...
            if (!ALLOWED_CSS_VARS.has(name)) {
              ...
              console.warn(
                '[StyleOverride] Rejected unauthorized CSS variable:',
                name
              );
              return;
            }
            ...
            if (!isValidCSSValue(name, value)) {
              rejectedVars.push(name);
              console.warn(
                '[StyleOverride] Rejected invalid CSS value:',
                name,
                value
              );
              return;
