---
title: "Preview"
description: "Set up a dev server, preview your app in the built-in browser, and inspect components across frameworks"
---
...
Click **Edit Dev Server Script**, enter your dev server command, and click **Save and Test** to verify it works.

<Frame>
  <img src="/images/workspaces-preview-script-dialog.png" alt="Script editor dialog with dev server command and Save and Test button" />
</Frame>

**Configuring the script:**

1. The script input field — enter the command that starts your dev server (e.g. `npm run dev`, `pnpm dev`, `yarn dev`)
2. The **Save and Test** button — saves the script and immediately starts the dev server so you can verify the configuration
...
Logs stream in real-time in the **Log** panel on the right so you can see when the server is ready.

<Frame>
  <img src="/images/workspaces-preview-dev-server-running.png" alt="Dev server log output streaming in real-time" />
</Frame>

<Tip>
Make sure your dev server prints its URL (e.g. `http://localhost:3000`) to stdout. Vibe Kanban detects this automatically and loads it in the preview.
</Tip>
...
1. **Back** — navigate to the previous page
2. **Forward** — navigate to the next page
3. **Inspect** (crosshair icon) — enter click-to-component mode
4. **DevTools** (terminal icon) — toggle the built-in devtools panel
5. **URL bar** — shows the current page address; type a URL to navigate manually
6. **URL actions** — copy the URL, open in a new tab, or refresh the page
7. **Device modes** — switch between Desktop, Mobile, and Responsive views
...
The preview browser includes a built-in click-to-component feature. Click the crosshair icon in the toolbar to enter inspect mode, hover over any element to see which component rendered it, then click to select.

<Frame>
  <img src="/images/workspaces-preview-inspect.png" alt="Inspect mode active with a component highlighted and its name shown in a tooltip" />
</Frame>

This works automatically with React, Vue, Svelte, Astro, and plain HTML — no packages to install. When you click a component, inspect mode exits and the component's name, file path, and location are captured.
