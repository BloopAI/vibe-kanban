...
|--------|----------|-------------|
| **Send / Steer** | `Cmd/Ctrl + Enter` | Send immediately (while running, this is high-priority steer) |
| **Queue** | `Shift + Enter` (running) or click Queue button | Buffer a follow-up while agent is running |
| **Edit Last Queued** | `Alt + ↑` | Pop the latest queued message back into the editor |
...
| **Queued** | One or more follow-ups are pending (`steer` and/or buffered `queue`) |
| **Sending** | Message is being sent |

<Info>
When the agent is running:
- **Send** inserts an immediate **steer** message.
- **Shift + Enter** (or **Queue**) appends a buffered follow-up.
- Buffered queue runs in FIFO order.
- Steer messages are consumed before buffered queue messages.
- **Alt + ↑** restores the latest queued message so you can edit and resend it.
</Info>
...
| Shortcut | Action |
|----------|--------|
| `Cmd/Ctrl + Enter` | Send message (running state: steer) |
| `Shift + Enter` | Queue follow-up while running |
| `Alt + ↑` | Edit (restore) latest queued message |
