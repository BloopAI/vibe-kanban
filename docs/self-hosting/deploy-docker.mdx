...
# User whitelist (optional — leave empty to allow all users)
# Comma-separated list of allowed email addresses
ALLOWED_USERS=admin@example.com,user@example.com
```

<Info>
**User whitelist**: Set `ALLOWED_USERS` to restrict access to specific email addresses (e.g., `admin@company.com,user@company.com`). When configured, only users with matching emails can authenticate. Leave empty to allow all users.
</Info>
