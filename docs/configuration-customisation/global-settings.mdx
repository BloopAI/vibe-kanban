...
For users running Vibe Kanban on remote servers, webhook notifications enable external notification delivery when tasks complete, fail, or are cancelled.

**Supported Platforms:**
- **Slack** - Send task completion notifications to Slack channels
- **Discord** - Post task updates to Discord channels
- **Pushover** - Receive push notifications on mobile devices
- **Telegram** - Send task notifications via Telegram bot
- **Generic** - Connect to any custom webhook endpoint

**Setup:**
1. Navigate to **Settings → Webhook Notifications**
2. Toggle "Enable Webhook Notifications" to receive task completion alerts
...
<Tip>
You can configure multiple webhooks to receive task notifications across different platforms simultaneously.
</Tip>

<Card title="Webhook Setup Guides" icon="webhook" href="/configuration-customisation/webhook-notifications">
  Complete guide to setting up webhook notifications for Slack, Discord, Pushover, Telegram, and custom endpoints
</Card>
