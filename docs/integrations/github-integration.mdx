...
If you see this error, ensure `gh` is available in your system's PATH. You can verify this by running `gh --version` in your terminal.

### "Insufficient permissions"
If you encounter permission errors:
1.  Run `gh auth status` to check your current authentication status.
2.  If needed, re-authenticate with `gh auth login` and ensure you grant the necessary permissions (specifically `repo` access) to the GitHub CLI.

### "Repository not found"
Ensure that the user you are authenticated as has access to the repository you are working on.
