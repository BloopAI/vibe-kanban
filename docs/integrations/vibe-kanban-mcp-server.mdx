...
| `create_and_start_sub_task` | Create a subtask and immediately start execution | `parent_task_attempt_id`<br/>`title`<br/>`executor` | `description`<br/>`variant`<br/>`base_branch` | Task ID, attempt ID, and parent ID |

### Log Monitoring

| Tool | Purpose | Required Parameters | Optional Parameters | Returns |
|------|---------|-------------------|-------------------|---------|
| `tail_task_log` | Fetch or stream execution logs | `task_attempt_id` | `lines`<br/>`follow` | Log lines and optional WebSocket URL |
...
### Creating and Starting Subtasks

When working on a complex task, you can create subtasks that link back to the parent task:

```
Create a subtask for implementing the user authentication API using Claude Code
```

Your MCP client will use the `create_and_start_sub_task` tool with parameters like:
```json
{
  "parent_task_attempt_id": "123e4567-e89b-12d3-a456-426614174000",
  "title": "Implement user authentication API",
  "executor": "claude-code",
  "description": "Create REST endpoints for user registration, login, and logout"
}
```

### Monitoring Task Execution Logs

To check the progress of a running task:

```
Show me the last 50 lines of logs from task attempt 456e7890-f12c-34d4-b567-537715289111
```

Or to follow logs in real-time:

```
Follow the logs for the currently running task attempt
```

The `tail_task_log` tool can return historical logs or provide a WebSocket URL for real-time log streaming.
...
Each step uses the appropriate MCP tool (`list_projects`, `list_tasks`, `create_task`, `start_task_attempt`, `create_and_start_sub_task`, `tail_task_log`) to manage the complete workflow from planning to execution.
