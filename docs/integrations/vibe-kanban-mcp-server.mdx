...
In orchestrator mode, the MCP server now derives its orchestrator session from the active workspace context automatically. If you only specify `--mcp`, the server defaults to `global` mode.
...
|------|---------|-------------------|-------------------|---------|
| `get_context` | Get current workspace context (only available within an active workspace session) | None | None | Project, issue, workspace, and orchestrator-session metadata |
