...
In orchestrator mode, the MCP server now derives its attached session from the active workspace context automatically. If you only specify `--mcp`, the server defaults to `global` mode.
