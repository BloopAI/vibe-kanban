...
| `get_session` | Get details for a single session | `session_id` | None | Session summary |
| `run_session_prompt` | Run a coding-agent prompt inside an existing session | `session_id`<br/>`prompt` | `executor`<br/>`variant`<br/>`model_id`<br/>`retry_process_id` | Execution details |
| `session_queue_message` | Queue a follow-up message for a session | `session_id`<br/>`message`<br/>`executor` | `variant`<br/>`model_id` | Queue status |
| `session_get_queue` | Inspect the queued follow-up status for a session | `session_id` | None | Queue status |
| `session_cancel_queue` | Cancel a queued follow-up for a session | `session_id` | None | Queue status |
...
| `respond_approval` | Approve or deny a pending tool approval request | `approval_id`<br/>`execution_id`<br/>`status` | `reason` | Approval result |
