...
Comma-separated list of files, directories, or glob patterns to copy from the original project directory to the worktree. These files will be copied after the worktree is created but before the setup script runs. You can specify individual files (`.env`), entire directories (`secrets/`), or glob patterns (`*.env`, `config/**/*.yml`).
