...
function copyFileIfExists(src, dest) {
  if (fs.existsSync(src)) {
    ...
    log(`Copied ${path.basename(src)} to ${path.relative(process.cwd(), dest)}`);
    return true;
  }
  return false;
}
...
function copyDirIfExists(src, dest) {
  if (fs.existsSync(src)) {
    ...
    log(`Copied directory ${path.basename(src)} to ${path.relative(process.cwd(), dest)}`);
    return true;
  }
  return false;
}
...
function ensureCargoTools() {
  try {
    ...
    try {
      ...
      error('Failed to install cargo-watch. Please install manually: cargo install cargo-watch');
      process.exit(1);
    }
  }
  ...
  try {
    ...
    try {
      ...
      error('Failed to install sqlx-cli. Please install manually: cargo install sqlx-cli');
      process.exit(1);
    }
  }
}
  ...
  if (args.length === 0) {
    ...
    try {
      ...
      if (gitDir.includes('worktrees')) {
        ...
        const mainGitDir = execSync('git rev-parse --git-common-dir', { encoding: 'utf8' }).trim();
        const mainRepoPath = path.dirname(mainGitDir);
        const currentPath = process.cwd();
        
        setupWorktree(currentPath, mainRepoPath);
      } else {
        error('Not in a git worktree. Usage: node setup-agent-worktree.js [worktree-path] [main-repo-path]');
        process.exit(1);
      }
    } catch (e) {
    ...
    console.log('Usage:');
    console.log('  node setup-agent-worktree.js                           - Auto-setup current worktree');
    console.log('  node setup-agent-worktree.js <worktree> <main-repo>    - Setup specific worktree');
    process.exit(1);
  }
}
