...
<p align="center">Holen Sie 10X mehr aus Claude Code, Gemini CLI, Codex, Amp und anderen KI-Coding-Agenten heraus...</p>
<p align="center">
...
- Orchestrierung der Ausführung mehrerer Coding-Agenten parallel oder sequenziell
- Schnelles Überprüfen von Arbeit und Starten von Entwicklungsservern
- Verfolgung des Status von Aufgaben, an denen Ihre Coding-Agenten arbeiten
- Zentralisierte Konfiguration von MCP-Configs für Coding-Agenten
- Fernzugriff auf Projekte via SSH, wenn Vibe Kanban auf einem Remote-Server läuft
...
Wir verwenden [GitHub Discussions](https://github.com/BloopAI/vibe-kanban/discussions) für Feature-Anfragen. Bitte eröffnen Sie eine Diskussion, um eine Feature-Anfrage zu erstellen. Für Bugs öffnen Sie bitte ein Issue in diesem Repository.
...
| `PORT` | Laufzeit | Automatisch | **Produktion**: Server-Port. **Entwicklung**: Frontend-Port (Backend verwendet PORT+1) |
| `BACKEND_PORT` | Laufzeit | `0` (automatisch) | Backend-Server-Port (nur im Entwicklungsmodus, überschreibt PORT+1) |
| `FRONTEND_PORT` | Laufzeit | `3000` | Frontend-Entwicklungsserver-Port (nur im Entwicklungsmodus, überschreibt PORT) |
| `HOST` | Laufzeit | `127.0.0.1` | Backend-Server-Host |
| `DISABLE_WORKTREE_ORPHAN_CLEANUP` | Laufzeit | Nicht gesetzt | Deaktiviert Git-Worktree-Bereinigung (für Debugging) |
...
Wenn Sie Vibe Kanban auf einem Remote-Server ausführen (z.B. via systemctl, Docker oder Cloud-Hosting), können Sie Ihren Editor so konfigurieren, dass Projekte via SSH geöffnet werden:

1. **Zugriff via Tunnel**: Verwenden Sie Cloudflare Tunnel, ngrok oder ähnliches, um die Web-Oberfläche freizugeben
2. **Remote-SSH konfigurieren** unter Einstellungen → Editor-Integration:
