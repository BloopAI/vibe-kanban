...
impl ExecutionProcess {
    ...
    /// Find latest coding_agent_turn session info (session_id and message_uuid) by session
    pub async fn find_latest_coding_agent_turn_session_info(
        pool: &SqlitePool,
        session_id: Uuid,
    ) -> Result<Option<(String, Option<String>)>, sqlx::Error> {
        tracing::info!(
            "Finding latest coding agent turn session info for session {}",
            session_id
        );
        let row = sqlx::query!(
            r#"SELECT cat.agent_session_id, cat.agent_message_uuid
               FROM execution_processes ep
               JOIN coding_agent_turns cat ON ep.id = cat.execution_process_id
               WHERE ep.session_id = $1
                 AND ep.run_reason = 'codingagent'
        ...
        tracing::info!("Latest coding agent turn session info: {:?}", row);

        Ok(row.and_then(|r| r.agent_session_id.map(|sid| (sid, r.agent_message_uuid))))
    }
