...
use super::tracked_pr::TrackedPr;
...
/// Row type for direct merges only (PR data now lives in tracked_prs).
#[derive(FromRow)]
...
impl Merge {
    ...
    /// Direct merges come from the `merges` table, PRs from `tracked_prs`.
    pub async fn find_by_workspace_id(
        ...
        let tracked_prs = TrackedPr::find_by_workspace_id(pool, workspace_id).await?;

        let mut merges: Vec<Merge> = direct_rows.into_iter().map(|row| row.into()).collect();
        merges.extend(tracked_prs.iter().map(|pr| pr.to_merge()));
    ...
    pub async fn find_by_workspace_and_repo_id(
        ...
        let tracked_prs =
            TrackedPr::find_by_workspace_and_repo_id(pool, workspace_id, repo_id).await?;
