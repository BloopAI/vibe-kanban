...
/// Placeholder title used when creating a task without a title.
/// The AI agent will generate an appropriate title based on the task description.
pub const PLACEHOLDER_TITLE: &str = "[AI will generate title]";

#[derive(Debug, Clone, Serialize, Deserialize, TS)]
pub struct CreateTask {
    pub project_id: Uuid,
    /// Task title. If empty or None, a placeholder will be used and AI will generate the title.
    pub title: String,
    pub description: Option<String>,
...
impl CreateTask {
    /// Returns the effective title - uses placeholder if title is empty
    ...
    /// Returns true if the title needs to be generated by AI
    pub fn needs_title_generation(&self) -> bool {
...
impl Task {
    /// Returns true if this task's title is a placeholder that needs AI generation
    pub fn needs_title_generation(&self) -> bool {
        self.title == PLACEHOLDER_TITLE || self.title.trim().is_empty()
    }

    pub fn to_prompt(&self) -> String {
        let needs_title = self.needs_title_generation();
        let description = self.description.as_ref().filter(|d| !d.trim().is_empty());

        let mut prompt = if needs_title {
            // When title needs generation, only use description as the main task
            if let Some(desc) = description {
                desc.clone()
            } else {
                // Edge case: no title and no description
                "Please provide a task description.".to_string()
            }
        ...
        // Add instruction to generate title if needed
        if needs_title {
            prompt.push_str("\n\n---\nIMPORTANT: After understanding the task, please update this task's title using the `update_task` MCP tool with a concise, descriptive title (max 60 characters) that summarizes what needs to be done. Use the task_id from the current context.");
        }
...
ORDER BY t.created_at DESC"#,
    ...
    pub async fn create(
        ...
        // Use effective_title to handle empty titles with placeholder
        let title = data.effective_title();
        sqlx::query_as!(
            Task,
