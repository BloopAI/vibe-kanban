...
#[derive(Debug, Clone, Serialize, Deserialize, TS)]
pub struct GlobalTaskWithAttemptStatus {
    #[serde(flatten)]
    #[ts(flatten)]
    pub task: Task,
    pub has_in_progress_attempt: bool,
...
impl std::ops::Deref for GlobalTaskWithAttemptStatus {
    type Target = Task;
    fn deref(&self) -> &Self::Target {
        &self.task
    }
...
impl std::ops::DerefMut for GlobalTaskWithAttemptStatus {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.task
    }
}
...
ORDER BY t.created_at DESC"#,
    ...
    pub async fn find_all_with_attempt_status_and_project_name(
        pool: &SqlitePool,
    ) -> Result<Vec<GlobalTaskWithAttemptStatus>, sqlx::Error> {
        let records = sqlx::query!(
            r#"SELECT
  t.id                            AS "id!: Uuid",
  t.project_id                    AS "project_id!: Uuid",
...
ORDER BY t.created_at DESC"#
        ...
        let tasks = records
            .into_iter()
            .map(|rec| GlobalTaskWithAttemptStatus {
                task: Task {
                    id: rec.id,
                    project_id: rec.project_id,
                    title: rec.title,
