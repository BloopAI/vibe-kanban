...
async fn check_externally_deleted_worktrees(pool: &sqlx::SqlitePool) {
    let active_attempts = match TaskAttempt::find_by_worktree_deleted(pool).await {
        Ok(attempts) => attempts,
        Err(e) => {
            tracing::error!(
                "Failed to query active task attempts for external deletion check: {}",
                e
            );
            return;
        }
...
async fn cleanup_orphaned_worktrees(db: &DBService) {
    ...
    // todo
    let worktree_base_dir = PathBuf::from("/path/to/worktree/base");
...
pub async fn execution_monitor(deployment: DeploymentImpl) {
    ...
    loop {
        ...
        check_externally_deleted_worktrees(&deployment.db().pool).await;

        // Then, find and delete orphaned worktrees that don't belong to any task
        cleanup_orphaned_worktrees(&deployment.db()).await;

        // Then, proceed with normal expired worktree cleanup
        match TaskAttempt::find_expired_for_cleanup(&deployment.db().pool).await {
            Ok(expired_attempts) => {
                if expired_attempts.is_empty() {
                    ...
                    for (attempt_id, worktree_path, git_repo_path) in expired_attempts {
                        if let Err(e) =
                            delete_worktree(worktree_path, git_repo_path, attempt_id).await
                        {
                            tracing::error!(
                                "Failed to cleanup expired worktree {}: {}",
                                attempt_id,
                                e
                            );
                        } else {
                                ...
                                .await
                            {
                                ...
                                tracing::info!(
                                    "Successfully marked worktree as deleted for attempt {}",
                                    attempt_id
                                );
                            }
                        }
