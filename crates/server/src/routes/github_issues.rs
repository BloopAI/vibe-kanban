...
use services::services::github::{GitHubIssue, GitHubService};
use ts_rs::TS;
...
pub async fn list_github_issues(
    ...
    // Get GitHub repo info from the repo path
    let repo_info = deployment.git().get_github_repo_info(&repo.path).map_err(|e| {
        ApiError::BadRequest(format!(
            "Repository is not a GitHub repository or remote not configured: {e}"
        ))
    })?;

    // Create GitHub service and fetch issues
    let github_service = GitHubService::new().map_err(|e| {
        ApiError::BadRequest(format!("Failed to initialize GitHub service: {e}"))
    })?;

    let issues = github_service
        .list_issues(&repo_info, &query.state)
        .await?;
