...
use db::models::{merge::MergeStatus, tracked_pr::TrackedPr, workspace::Workspace};
...
pub async fn link_workspace(
...

    {
        ...
        tokio::spawn(async move {
            let tracked_prs = match TrackedPr::find_by_workspace_id(&pool, ws_id).await {
                Ok(prs) => prs,
                Err(e) => {
                    tracing::error!(
                        "Failed to fetch tracked PRs for workspace {} during link: {}",
                        ws_id,
                        e
                    );
                    return;
            ...
            for tracked_pr in tracked_prs {
                let pr_status = match tracked_pr.pr_status {
