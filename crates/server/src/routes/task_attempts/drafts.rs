...
use crate::{
    ...
    error::ApiError,
    services::drafts::{
        DraftResponse, DraftsService, SetQueueRequest, UpdateFollowUpDraftRequest,
...
pub async fn save_follow_up_draft(
    ...
    let service = DraftsService::new(&deployment);
    let resp = service
...
pub async fn set_follow_up_queue(
    ...
    let service = DraftsService::new(&deployment);
    let resp = service.set_follow_up_queue(&task_attempt, &payload).await?;
    Ok(ResponseJson(ApiResponse::success(resp)))
}
