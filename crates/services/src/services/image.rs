...
impl ImageService {
    ...
    pub async fn attach_to_task(&self, image_id: Uuid, task_id: Uuid) -> Result<(), ImageError> {
        Image::set_task_id(&self.pool, image_id, Some(task_id)).await?;
        Ok(())
    }
    ...
    pub async fn get_task_images(&self, task_id: Uuid) -> Result<Vec<Image>, ImageError> {
        Ok(Image::find_by_task_id(&self.pool, task_id).await?)
    }
