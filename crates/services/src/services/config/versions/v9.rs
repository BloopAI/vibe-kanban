...
use crate::services::git_host::ProviderKind;
...
/// Configuration for a single git host.
...
/// Configuration for git hosting providers.
#[derive(Clone, Debug, Default, Serialize, Deserialize, TS, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct GitHostsConfig {
    /// Map of host domain -> provider configuration.
    /// Example: "git.mycompany.com" -> { provider: Forgejo, token: "..." }
    #[serde(default)]
...
pub struct Config {
    ...
    /// Git hosting provider configuration for self-hosted instances.
    #[serde(default)]
    pub git_hosts: GitHostsConfig,
}
