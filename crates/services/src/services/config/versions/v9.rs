...
mod tests {
    ...
    fn test_v8_to_v9_migration() {
        ...
        let config = Config::from(v8_raw.to_string());
        assert_eq!(config.config_version, "v9");
        assert_eq!(config.notifications.custom_sound_path, None);
        // v8 config had CowMooing, so v9 should also have CowMooing
        assert_eq!(config.notifications.sound_file, SoundFile::CowMooing);
    }
}
