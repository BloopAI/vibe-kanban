...
fn merge_refuses_with_staged_changes_on_base() {
    ...
    let res = s.merge_changes(&repo_path, &worktree_path, "feature", "main", "squash", MergeStrategy::Strict);
...
fn merge_preserves_unstaged_changes_on_base() {
    ...
    let _sha = s
        .merge_changes(&repo_path, &worktree_path, "feature", "main", "squash", MergeStrategy::Strict)
        .unwrap();
...
fn merge_refreshes_main_worktree_when_on_base() {
    ...
    let merge_sha = s
        .merge_changes(&repo_path, &wt, "feature", "main", "squash", MergeStrategy::Strict)
        .unwrap();
...
fn merge_binary_conflict_does_not_move_ref() {
    ...
    let res = s.merge_changes(&repo_path, &worktree_path, "feature", "main", "merge bin", MergeStrategy::Strict);
